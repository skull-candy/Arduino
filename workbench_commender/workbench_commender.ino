...
String buttonText[04] = {
  "01. TOP Lamp","02. Room Lamp","03. Sockets","04. FAN"};
 ...
 //Setting up the IP address. Comment out the one you dont need.
  ...
  for (int var = 0; var < outputQuantity; var++){
    ...
    if(outputInverted == true) {
      ...
      if(outputStatus[var] == 1){currentState = true;}else{currentState = false;} //check outputStatus if off, switch output accordingly
      ...
      if(outputStatus[var] == 1){currentState = false;}else{currentState = true;}//check outputStatus if off, switch output accordingly
      digitalWrite(outputAddress[var], currentState);
    }
...
//  float celsius = (millivolts / 10) - 273.15 ;
  ...
  float celsius = mv/10 - 273.15;
//  float farh = (celsius*9)/5 + 32;
    float centi =(celsius-2);
    tempOutDeg = centi;
 // Serial.print(centi);
...
//Prints html header
 void printHtmlHeader(EthernetClient client){
          ...
          client.println("    background-color: #d3c5cc;");
          ...
          client.println("    background-color: #696266;");
          client.println("    background-image:");
