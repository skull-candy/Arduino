...
void loop()
{
  ...
  if (stringCommand.startsWith("2")) {
    if (backlitvS == 1)
    {
      digitalWrite (backlit , LOW);
      Serial.println("backlitv De-Energized (Console Command)");
      backlitvS = 0;
    }
    else
    {
      Serial.println("backlitv is Already OFF");
    }
  }
  if (stringCommand.startsWith("report") || stringCommand.startsWith("?")) {
    Serial.println(backlitvS);
  }
